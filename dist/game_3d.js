...
function sliceAndCreateCakePieces() {
  ...
  for (let i = 0; i < numSlices; i++) {
    ...
    // ケーキをスライス
    const sliceCSG = cakeCSG.cutByPlane(planeA).cutByPlane(planeB);
...
function createAnt() {
  ...
  const ant = {
    ...
    targetPieceId: cakePieces[Math.floor(Math.random() * cakePieces.length)].id,
...
function handleSubmit(event) {
  ...
  // 読みが一致するアリを検索
  const matchingAnt = ants.find((ant) => ant.reading === userInput);
